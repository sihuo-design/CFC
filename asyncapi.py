...
async def process_api_requests_from_file(
    ...
    """Processes API requests sequentially, throttling to stay under rate limits."""
    # constants
    seconds_to_pause_after_rate_limit_error = seconds_to_pause
    seconds_to_sleep_each_loop = seconds_to_sleep  # 1 ms limits max throughput to 1,000 requests per second
    ...
    with open(requests_filepath) as file:
        requests = iter(file)
        logging.debug("File opened. Starting processing.")
        ...
        while True:
            ...
            if next_request:
                ...
                if available_request_capacity >= 1 and available_token_capacity >= next_request_tokens:
                    ...
                    next_request = None  # 继续下一个请求

            # **确保所有请求处理完毕才退出**
            if not file_not_finished and queue_of_requests_to_retry.empty():
                break

            await asyncio.sleep(seconds_to_sleep_each_loop)
            
            seconds_since_rate_limit_error = time.time() - status_tracker.time_of_last_rate_limit_error
            if seconds_since_rate_limit_error < seconds_to_pause:
                remaining_seconds_to_pause = seconds_to_pause - seconds_since_rate_limit_error
                await asyncio.sleep(remaining_seconds_to_pause)
                logging.warning(f"Pausing until {time.ctime(status_tracker.time_of_last_rate_limit_error + seconds_to_pause)}")

        logging.info(f"Processing complete. Results saved to {save_filepath}")
        if status_tracker.num_tasks_failed > 0:
            logging.warning(f"{status_tracker.num_tasks_failed} / {status_tracker.num_tasks_started} requests failed.")
        if status_tracker.num_rate_limit_errors > 0:
            logging.warning(f"{status_tracker.num_rate_limit_errors} rate limit errors received.")
