...
def filter_category_by_TFIDT(category_list, count_list):
    # # Compare similarity using fuzzy matching
    # for i in range(len(category_list)):
    #     for j in range(i + 1, len(category_list)):
    #         similarity = fuzz.ratio(category_list[i], category_list[j])
    #         print(f'Fuzzy similarity between {category_list[i]} and {category_list[j]}: {similarity}')  

    # for i in range(len(category_list)):
    #     for j in range(i + 1, len(category_list)):
    #         similarity = jaccard_similarity(category_list[i], category_list[j])
    #         print(f'Jaccard similarity between {category_list[i]} and {category_list[j]}: {similarity:.2f}')

    # # Compare similarity using Levenshtein distance
    # for i in range(len(category_list)):
    #     for j in range(i + 1, len(category_list)):
    #         similarity = Levenshtein.ratio(category_list[i], category_list[j])
    #         print(f'Similarity between {category_list[i]} and {category_list[j]}: {similarity:.2f}')
    
    # Convert the strings to vectors using TF-IDF
    vectorizer = TfidfVectorizer().fit_transform(category_list)
    ...
    for i in range(len(category_list)):
        for j in range(i + 1, len(category_list)):
            print(f'Cosine similarity between {category_list[i]} and {category_list[j]}: {cosine_sim[i][j]:.2f}')
            if cosine_sim[i][j] > 0.3:
                if len(category_list[i]) > len(category_list[j]):
                    category_list[j] = category_list[i]
                    count_list[j] = count_list[i] + count_list[j]
                    count_list[i] = 0
                else:
                    print("replace "+category_list[i]+" by "+category_list[j])
                    category_list[i] = category_list[j]   
                    count_list[j] = count_list[i] + count_list[j]   
                    count_list[i] = 0 
    mask = np.array(count_list) > 0   
    return np.array(category_list)[mask], np.array(count_list)[mask]


def filter_category_by_TFIDT_dict(category_list, count_list):
    # # Compare similarity using fuzzy matching
    # for i in range(len(category_list)):
    #     for j in range(i + 1, len(category_list)):
    #         similarity = fuzz.ratio(category_list[i], category_list[j])
        ...
        for j in range(i + 1, len(category_list)):
            ...
            if cosine_sim[i][j] > 0.3:
                if len(category_list[i]) > len(category_list[j]):
                    ...
                    print("replace "+category_list[i]+" by "+category_list[j])
                    dict_replace[category_list[i]] = category_list[j]
                    category_list[i] = category_list[j]   
                    count_list[j] = count_list[i] + count_list[j]   
                    count_list[i] = 0 
    mask = np.array(count_list) > 0   
    return np.array(category_list)[mask], np.array(count_list)[mask], dict_replace
    
def replace_category_by_annotation(category_list_llm, category_list_anno):

    all_categories = category_list_llm + category_list_anno
    

    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(all_categories)  #

    llm_tfidf = tfidf_matrix[:len(category_list_llm)]  # 取出 llm 部分的TF-IDF向量
    anno_tfidf = tfidf_matrix[len(category_list_llm):]  # 取出 anno 部分的TF-IDF向量
    
    similarity_matrix = cosine_similarity(llm_tfidf, anno_tfidf)  # 计算余弦相似度
    

    closest_categories = []

    for i, llm_category in enumerate(category_list_llm):

        closest_idx = np.argmax(similarity_matrix[i])
        if similarity_matrix[i][closest_idx] > 0.3:
            closest_categories.append(category_list_anno[closest_idx])
            
        else:         
            closest_categories.append(category_list_llm[i])
    
    return closest_categories

def replace_category_by_annotation_dict(category_list_llm, category_list_anno, dict_replace):
    ...
    for i, llm_category in enumerate(category_list_llm):
        ...
        if similarity_matrix[i][closest_idx] > 0.3:
            closest_categories.append(category_list_anno[closest_idx])
            dict_replace[llm_category] = category_list_anno[closest_idx]
        else:         
            closest_categories.append(category_list_llm[i])
    
    return closest_categories, dict_replace