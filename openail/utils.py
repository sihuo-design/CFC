...
def efficient_openai_text_api(input_text, filename, savepath, sp, ss, api_key="change_this_to_your_key", request_url = "https://huggingface.co/meta-llama/Meta-Llama-3-8B", rewrite = True, temperature = 0, n = 1):
    ...
    if not osp.exists(savepath) or rewrite:
        jobs = generate_chat_input_file(input_text, model_name="meta-llama/Meta-Llama-3-8B", temperature = temperature, n = n)
...
def efficient_llama_text_api(input_text, filename, savepath, sp, ss, api_key="change_this_to_your_key", request_url = "https://huggingface.co/meta-llama/Meta-Llama-3-8B", rewrite = True, temperature = 0, n = 1):
    # import ipdb; ipdb.set_trace()
    # openai_result = []
    rewrite = True
    non_empty_results = []
    ...
    #     tokenizer.pad_token = tokenizer.eos_token
    for i, job in enumerate(input_text):
        if job != "":
        ...
        #         response = generator(job['messages'][0]['content'])
        #         choices = [x['generated_text'] for x in response]
